...
function setupAlbumFeature() {
    ...
    backToAlbumsBtn.addEventListener('click', () => {
        ...
        albumModal.style.display = 'flex'; // 返回相册列表
        // 恢复轮播图滑动
        isPaused = false;
        startCarousel();
    });
    ...
    document.addEventListener('keydown', function closePhotosOnEscape(e) {
        if (e.key === 'Escape' && photosModal.style.display === 'flex') {
            photosModal.style.display = 'none';
            albumModal.style.display = 'flex'; // 返回相册列表
            // 恢复轮播图滑动
            isPaused = false;
...
function setupAlbumFeature() {
    ...
    // 设置轮播图相册按钮
    const setXiaoyemaoBtn = document.getElementById('set-xiaoyemao');
    const setXinchunBtn = document.getElementById('set-xinchun');
    
    setXiaoyemaoBtn.addEventListener('click', () => {
        setCurrentAlbum("小野猫");
    });
    
    setXinchunBtn.addEventListener('click', () => {
        setCurrentAlbum("新春快乐");
    });
}
...
function setCurrentAlbum(albumName) {
    ...
    if (albumName === "小野猫") {
        setXiaoyemaoBtn.classList.add('active');
        setXinchunBtn.classList.remove('active');
    } else if (albumName === "新春快乐") {
        setXinchunBtn.classList.add('active');
        setXiaoyemaoBtn.classList.remove('active');
    }
    ...
    // 显示提示信息
    // alert(`已设置轮播图为"${albumName}"相册`); // 注释掉提示信息，避免干扰用户体验
}
...
>>>>>>> 39c6a9d78058c0215d2a4627c080f6fd5f3f1174
